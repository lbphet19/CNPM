...
//			return ResponseEntity.ok(new ResponseDTO(true,hoKhauUpdate));
	...
	@PutMapping("/hoKhau")
	public ResponseEntity<ResponseDTO> put(@RequestBody SoHoKhauRequestDTO soHoKhauRequestDTO) throws OwnerNotAvailableException, CongDanQuaDoiException{
		//Check cong dan qua doi
		congDanService.checkPassedAwayByID(Integer.parseInt(soHoKhauRequestDTO.getId()));
		List<CongDanOfSHKRequestDTO> listcd=soHoKhauRequestDTO.getMembers();
		for(int i=1;i<=listcd.size();i++) {congDanService.checkPassedAwayByID(listcd.get(i).getId()); }
		/*****************/
