...
public class KhaiBaoController {
	...
	public ResponseEntity<ResponseDTO> postTT(@RequestBody KhaiBao kbtt){
		try {
			...
			congDanService.checkPassedAwayByCCCD(kbtt.getCanCuocCongDan());
	...
	public ResponseEntity<ResponseDTO> postTV(@RequestBody KhaiBao kbtv){
		try {
			congDanService.checkPassedAwayByCCCD(kbtv.getCanCuocCongDan());
			...
			this.shkHistoryService.save(new SoHoKhauHistory("Tạm vắng", Date.valueOf(LocalDate.now()),
					"Công dân " + congDanUpdate.getFirstName() + " " + congDanUpdate.getLastName() + " đăng ký tạm vắng"+
